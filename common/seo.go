...
func PublicSEOPages() []SEOPage {
	return []SEOPage{
		...
		},
		{
			Path:        "/docs",
			Title:       "Documentation",
			Description: "Guides for BoxAI on you-box.com — console, API, clients, and Playground.",
			Priority:    "0.85",
			Changefreq:  "weekly",
			Heading:     "BoxAI Documentation",
			Body:        "Task-oriented guides for the BoxAI unified AI API gateway at you-box.com: getting started, console, API protocols, clients, and Playground.",
		},
		{
		...
		{
			Path:        "/docs/start/first-request",
			Title:       "First request",
			Description: "Send a successful chat completion from Playground or curl on BoxAI (you-box.com).",
			Priority:    "0.8",
			Changefreq:  "weekly",
			Heading:     "First request",
			Body:        "Complete one successful model call on BoxAI via Playground or the OpenAI-compatible API, then confirm usage logs.",
		},
		{
			Path:        "/docs/console/api-keys",
			Title:       "Create and manage API keys",
			Description: "Issue a BoxAI API key, set limits, and call the gateway from your server.",
			Priority:    "0.75",
			Changefreq:  "weekly",
			Heading:     "Create and manage API keys",
			Body:        "Create, rotate, and revoke BoxAI API keys. Keep secrets on your server and never embed them in public frontends.",
		},
		{
			Path:        "/docs/console/model-hub",
			Title:       "Model Hub",
			Description: "Find exact model IDs and pricing on the BoxAI Model Hub at you-box.com/pricing.",
			Priority:    "0.75",
			Changefreq:  "weekly",
			Heading:     "Model Hub",
			Body:        "Browse BoxAI model IDs, pricing, and capabilities before integrating with the unified API gateway.",
		},
		{
			Path:        "/docs/console/billing-topup",
			Title:       "Billing and top-up",
			Description: "Top up BoxAI balance with supported payment rails, including Vietnam-first options.",
			Priority:    "0.7",
			Changefreq:  "weekly",
			Heading:     "Billing and top-up",
			Body:        "Check balance and top up BoxAI with supported payment methods for your region.",
		},
		{
			Path:        "/docs/console/usage-logs",
			Title:       "Usage logs",
			Description: "Read BoxAI request history, tokens, and status codes for debugging and billing.",
			Priority:    "0.7",
			Changefreq:  "weekly",
			Heading:     "Usage logs",
			Body:        "Inspect BoxAI usage logs to debug integrations and understand quota consumption.",
		},
		{
			Path:        "/docs/api/overview",
			Title:       "API overview",
			Description: "BoxAI gateway base URL, protocol profiles, and model IDs from Model Hub.",
			Priority:    "0.8",
			Changefreq:  "weekly",
			Heading:     "API overview",
			Body:        "Call the BoxAI gateway at https://you-box.com with protocol-specific profiles and exact model IDs from Model Hub.",
		},
		{
			Path:        "/docs/api/auth",
			Title:       "API authentication",
			Description: "Authenticate BoxAI gateway calls with Bearer keys or profile-specific headers.",
			Priority:    "0.75",
			Changefreq:  "weekly",
			Heading:     "API authentication",
			Body:        "Authenticate to BoxAI with API keys. Prefer server-side callers and rotate leaked credentials immediately.",
		},
		{
			Path:        "/docs/streaming",
			Title:       "Streaming",
		...
		},
		{
			...
			Title:       "BoxAI Desktop",
			Description: "Install BoxAI Desktop on macOS or Windows and sign in with your BoxAI account.",
			Priority:    "0.7",
			Changefreq:  "weekly",
			Heading:     "BoxAI Desktop",
		...
		},
		{
			...
			Priority:    "0.7",
			Changefreq:  "weekly",
			Heading:     "Playground overview",
			Body:        "Use the BoxAI Playground to validate models and prompts before integrating the API.",
		},
		{
			Path:        "/docs/concepts/models-groups-quota",
			Title:       "Models, groups, and quota",
			Description: "How model IDs, access groups, and remaining quota work together on BoxAI.",
			Priority:    "0.65",
			Changefreq:  "monthly",
			Heading:     "Models, groups, and quota",
			Body:        "Understand BoxAI model IDs, group access, and quota before debugging permission or billing issues.",
		},
		{
			Path:        "/rankings",
			Title:       "Rankings",
...
func LookupSEOPage(path string) (SEOPage, bool) {
	...
	if strings.HasPrefix(path, "/docs/") && path != "/docs" {
		...
		// Nested guides (/docs/start/getting-started) and protocol pages are indexable.
		if slug != "" && !strings.Contains(slug, "..") {
			parts := strings.Split(slug, "/")
			last := parts[len(parts)-1]
			title := humanizeSlug(last)
			return SEOPage{
				Path:        path,
				Title:       title,
...
func InjectSEOIntoHTML(indexHTML []byte, page SEOPage, siteName, baseURL, imageURL string, noindex bool) []byte {
	...
	// Default shell locale is English; normalize leftover zh-CN shells.
	htmlStr = replaceOnce(htmlStr, `<html lang="zh-CN">`, `<html lang="en">`)
	htmlStr = replaceOnce(htmlStr, `<html lang="vi">`, `<html lang="en">`)
	...
	htmlStr = upsertMeta(htmlStr, "property", "og:locale", "en_US")
