...
type BodyStorage interface {
	...
	// stored payload. Closing it never closes the storage itself.
	NewReader() (io.ReadCloser, error)
}

// ReplayableBody is an outbound request body whose content can be replayed by
// net/http after a retryable transport failure.
...
// NewReplayableBodyReader exposes replay metadata without exposing io.Closer,
// leaving storage lifecycle ownership with the caller.
