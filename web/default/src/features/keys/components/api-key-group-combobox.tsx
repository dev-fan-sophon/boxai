...
function getRatioBadgeClassName(ratio: ApiKeyGroupOption['ratio']) {
  if (typeof ratio !== 'number') return tone('success', { bordered: true })
  if (ratio > 5) return tone('danger', { bordered: true })
  if (ratio > 3) return tone('warning', { bordered: true })
  if (ratio > 1) return tone('info', { bordered: true })
      ...
      <PopoverTrigger
        render={
          <Button
            ...
            aria-expanded={open}
            disabled={disabled}
            className='border-input bg-muted/40 hover:bg-muted/55 hover:text-foreground active:bg-background data-popup-open:border-ring data-popup-open:bg-background data-popup-open:ring-ring/20 duration-control h-auto min-h-14 w-full justify-between gap-2 rounded-lg px-3 py-2 text-start shadow-none transition-[background-color,border-color,box-shadow] data-popup-open:ring-[3px] sm:min-h-20 sm:gap-3 sm:px-4 sm:py-3'
          />
        }
      >
