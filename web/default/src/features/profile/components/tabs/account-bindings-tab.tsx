  ...
  const bindings: BindingItem[] = useMemo(() => {
    ...
    return [
      ...
      },
      {
        ...
        label: t('Facebook'),
        icon: IconFacebook,
        value: (profile as unknown as Record<string, unknown>).facebook_id as
          | string
          | undefined,
        isBound: Boolean(
          (profile as unknown as Record<string, unknown>).facebook_id
        ),
        isEnabled: status?.facebook_oauth || false,
        onBind: () => {
          if (status?.facebook_client_id) {
            handleFacebookOAuth(status.facebook_client_id)
          }
        },
      },
      {
        ...
        label: t('Zalo'),
        icon: IconZalo,
        value: (profile as unknown as Record<string, unknown>).zalo_id as
          | string
          | undefined,
        isBound: Boolean(
          (profile as unknown as Record<string, unknown>).zalo_id
        ),
        isEnabled: status?.zalo_oauth || false,
        onBind: () => {
          if (status?.zalo_app_id) {
            handleZaloOAuth(status.zalo_app_id)
          }
        },
      },
      {
