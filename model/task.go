...
import (
	...
	commonRelay "github.com/dev-fan-sophon/boxai/relay/common"
	"gorm.io/gorm"
)
...
func GetPendingTerminalTaskRefunds(limit int) []*Task {
	var tasks []*Task
	_ = DB.Where("quota != 0").Where("status IN ?", []string{TaskStatusFailure}).Order("id").Limit(limit).Find(&tasks).Error
	return tasks
}

func HasPendingTerminalTaskRefunds() bool {
	var count int64
...
type TaskPrivateData struct {
	...
	SubscriptionUsageGeneration int64               `json:"subscription_usage_generation,omitempty"`
...
func (p *TaskPrivateData) Scan(val interface{}) error {
	...
	p.SubscriptionUsageGeneration = UnknownSubscriptionUsageGeneration
