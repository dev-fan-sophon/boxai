...
type Midjourney struct {
	...
	TokenId                     int    `json:"-" gorm:"column:token_id"`
	BillingSource               string `json:"-" gorm:"type:varchar(32)"`
	SubscriptionId              int    `json:"-"`
	SubscriptionUsageGeneration int64  `json:"-" gorm:"not null;default:-1"`
	BillingOperationKey         string `json:"-" gorm:"type:varchar(191)"`
	BillingSettled              bool   `json:"-" gorm:"index;not null;default:false"`
	Buttons                     string `json:"buttons"`
	Properties                  string `json:"properties"`
}

func HasPendingMidjourneyRefunds() bool {
	var count int64
	return DB.Model(&Midjourney{}).Where("status = ? AND quota != 0", "FAILURE").Limit(1).Count(&count).Error == nil && count > 0
}

func PendingMidjourneyRefundTasks(limit int) []Midjourney {
	var rows []Midjourney
	_ = DB.Where("status = ? AND quota != 0", "FAILURE").Limit(limit).Find(&rows).Error
	return rows
}

func PendingMidjourneySettlementTasks(limit int) []Midjourney {
	var rows []Midjourney
	_ = DB.Where("status = ? AND billing_settled = ?", "SUCCESS", false).Limit(limit).Find(&rows).Error
	return rows
}

func HasPendingMidjourneySettlements() bool {
	var count int64
	return DB.Model(&Midjourney{}).Where("status = ? AND billing_settled = ?", "SUCCESS", false).Limit(1).Count(&count).Error == nil && count > 0
}
