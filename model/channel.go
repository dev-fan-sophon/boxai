...
// saveStatusState persists only fields owned by the channel-status flow. An
// allowlist prevents a stale channel snapshot from overwriting credentials,
// accounting counters, or concurrently edited configuration.
...
func UpdateChannelStatus(channelId int, usingKey string, status int, reason string) bool {
	...
	// ChannelInfo contains both per-key status and the polling cursor. Hold the
	// same per-channel lock from the first read through persistence so neither
	// status updates nor polling can save a stale JSON snapshot over the other.
