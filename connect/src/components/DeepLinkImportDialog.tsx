...
export function DeepLinkImportDialog() {
  ...
  useEffect(() => {
    ...
    const unlistenImport = listen<DeepLinkImportRequest>(
      "deeplink-import",
      async (event) => {
        // Connect does not accept third-party/BYOK provider imports — only MCP,
        // skills and prompts. Upstream CC Switch still ships provider deeplinks.
        const resource = event.payload.resource ?? "provider";
        if (resource === "provider") {
          toast.error(
            t("deeplink.providerImportDisabled", {
              defaultValue:
                "BoxAI Connect 不支持导入自定义供应商。请使用官方或 BoxAI 配置。",
            }),
          );
          return;
        }
