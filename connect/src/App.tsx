...
import { filterConnectProviders } from "@/utils/connectProviderPolicy";
import {
  isWindows,
  isLinux,
  DRAG_REGION_ATTR,
...
function App() {
  ...
  // In a curated client Connect surfaces official + BoxAI only; BYOK and
  // third-party stay out of the UI. Clients Connect does not seed keep their
  // full upstream surface.
  const providers = useMemo(
    () => filterConnectProviders(data?.providers ?? {}, activeApp),
    [data, activeApp],
  );
  ...
  // Backend seeds on sign-in and on cold start, then emits once. Skills use
  // staleTime: Infinity — refresh when that event lands.
  useTauriEvent("boxai-account-surface-updated", async () => {
    await queryClient.invalidateQueries({ queryKey: ["providers"] });
    ...
    try {
      ...
      console.error("[App] Failed to update tray menu after BoxAI seed", error);
    }
  });
            ...
            {currentView !== "providers" ? (
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  title={t("boxai.account.title", {
                    defaultValue: "BoxAI 账号",
                  })}
