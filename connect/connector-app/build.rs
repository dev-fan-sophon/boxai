...
fn main() {
    ...
    let metadata_bytes = fs::read(METADATA_PATH).expect("read BoxAI release metadata");
    let metadata: serde_json::Value =
        serde_json::from_slice(&metadata_bytes).expect("parse BoxAI release metadata");
    let version = field(&metadata, "version");
    assert_eq!(
        version,
        env::var("CARGO_PKG_VERSION").expect("Cargo package version"),
        "release metadata version must match the Cargo package version"
    );
    ...
    assert!(Path::new(ICON_PATH).is_file(), "BoxAI Windows icon missing");
    ...
    resource
        ...
        .compile()
        .expect("embed BoxAI Windows resources");
}
