...
mod tests {
    ...
    use super::*;
    use crate::{AppState, AsyncValue, ProjectionLifecycle, ProjectionSemantic};
    ...
    fn full_direct_lifecycle_stays_under_the_isolated_layout() {
        ...
        for (agent, selection) in &mut connection.profile.agents {
            selection.protocol = match agent {
                ...
                AgentId::Grokbuild | AgentId::Opencode | AgentId::Workbuddy => Protocol::OpenaiChat,
            };
        ...
        app_state.set_projection_status(
            AsyncValue::ready(installs),
            AsyncValue::ready(Default::default()),
        );
        ...
        let rejected_plan = backend.plan_projection(&connection).expect("plan");
        assert!(!rejected_plan.changes.is_empty());
        assert!(
            rejected_plan
                .changes
                .iter()
        ...
        let verification = backend.verify_projection(&applied_plan).expect("verify");
        ...
        let drift = backend
            .verify_projection(&applied_plan)
            .expect("drift report");
        assert!(!drift.ok);
        ...
        resumed_state.set_projection_status(
            AsyncValue::ready(resumed_backend.discover_agents().expect("resumed installs")),
            AsyncValue::ready(
                resumed_backend
