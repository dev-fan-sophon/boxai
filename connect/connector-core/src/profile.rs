...
/// The six Agent adapters supported by the shared projection contract.
...
pub enum AgentId {
    ...
    Grokbuild,
    Opencode,
    /// Tencent's Buddy family. Profiles written before the rename carry the
    /// `codebuddy` key, and so does a Gateway manifest that has not shipped
    /// the rename yet.
    #[serde(alias = "codebuddy")]
    Workbuddy,
}
...
impl AgentId {
    ...
    /// Reads an id back from persisted state. Receipts and leases written
    /// before the WorkBuddy rename spell that adapter `codebuddy`, and losing
    /// the match would strand a live projection with no owner.
    pub fn from_id(value: &str) -> Option<Self> {
        Self::ALL
            .into_iter()
            .find(|agent| agent.as_str() == value)
            .or((value == "codebuddy").then_some(Self::Workbuddy))
    }
    ...
    pub const fn supported_protocols(self) -> &'static [Protocol] {
        match self {
            ...
            Self::Workbuddy => &[Protocol::Auto, Protocol::OpenaiChat],
        }
    }

    /// Claude and Gemini accept `OPENAI_BASE_URL` / `OPENAI_API_KEY` in their
    /// projected env files. Codex uses its native Responses + image-generation
    /// path. Grok and OpenCode have no env channel.
    pub const fn image_direct_env(self) -> bool {
        matches!(self, Self::Claude | Self::Gemini)
    }

    pub const fn image_direct_native(self) -> bool {
    ...
    pub const fn supported_wire_protocols(self) -> &'static [WireProtocol] {
        match self {
            ...
            Self::Workbuddy => &[WireProtocol::OpenaiChat],
        }
    }
        ...
        impl FromStr for $name {
            ...
            fn from_str(value: &str) -> Result<Self, Self::Err> {
                Self::ALL
                    ...
                    .ok_or_else(|| ProfileError::InvalidCodexSetting {
...
codex_choice!(CodexReasoningEffort {
    None => "none",
    Minimal => "minimal",
    Low => "low",
    Medium => "medium",
...
codex_choice!(CodexReasoningSummary {
    Auto => "auto",
    Concise => "concise",
    Detailed => "detailed",
    None => "none",
});
codex_choice!(CodexVerbosity {
    Low => "low",
    Medium => "medium",
    High => "high",
});
codex_choice!(CodexApprovalPolicy {
    Untrusted => "untrusted",
    OnRequest => "on-request",
    Never => "never",
});
codex_choice!(CodexSandboxMode {
    ReadOnly => "read-only",
    WorkspaceWrite => "workspace-write",
    DangerFullAccess => "danger-full-access",
});
codex_choice!(CodexWebSearch {
    Disabled => "disabled",
    Cached => "cached",
    Indexed => "indexed",
    Live => "live",
...
/// Optional Codex-owned preferences. `None` preserves the user's existing
/// Codex value instead of replacing a client default with a Connector default.
...
pub struct CodexSettings {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub verbosity: Option<CodexVerbosity>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub approval_policy: Option<CodexApprovalPolicy>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub sandbox_mode: Option<CodexSandboxMode>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub web_search: Option<CodexWebSearch>,
    /// Extra chat models written beside the selected model in Codex
    /// `model_catalog_json`.
    #[serde(default, skip_serializing_if = "BTreeSet::is_empty")]
    pub catalog_models: BTreeSet<String>,
}
...
pub struct AgentSelection {
    ...
    /// Catalog MCP ids this Agent should not receive on the next Apply.
    /// Empty means every provisioned MCP is enabled.
    #[serde(default, skip_serializing_if = "BTreeSet::is_empty")]
    pub disabled_mcp: BTreeSet<String>,
    /// Catalog Skill ids this Agent should not receive on the next Apply.
    /// Empty means every provisioned Skill is enabled.
    #[serde(default, skip_serializing_if = "BTreeSet::is_empty")]
    pub disabled_skills: BTreeSet<String>,
    /// Codex-only user preferences projected to its native `config.toml`.
    #[serde(default, skip_serializing_if = "CodexSettings::is_empty")]
    pub codex: CodexSettings,
    /// Project Gateway Images API credentials into Agents that have an env
    /// channel (`OPENAI_BASE_URL` + `OPENAI_API_KEY`). Off by default.
...
impl AgentSelection {
    ...
    pub fn mcp_enabled(&self, id: &str) -> bool {
        !self.disabled_mcp.contains(id)
    }
    ...
    pub fn set_mcp_enabled(&mut self, id: impl Into<String>, enabled: bool) {
        let id = id.into();
        if enabled {
            self.disabled_mcp.remove(&id);
        } else {
            self.disabled_mcp.insert(id);
        }
    }

    pub fn set_skill_enabled(&mut self, id: impl Into<String>, enabled: bool) {
        let id = id.into();
        if enabled {
            self.disabled_skills.remove(&id);
        } else {
            self.disabled_skills.insert(id);
        }
    }
}
...
pub struct ConnectionProfile {
    ...
    /// The browser token was redeemed, but credential persistence has not yet
...
impl ConnectionProfile {
    ...
    pub fn reconfigured(
        ...
        for selection in existing.agents.values_mut() {
            ...
            selection.disabled_mcp.clear();
            selection.disabled_skills.clear();
            selection.image_direct = false;
        }
    ...
    pub fn validate(&self) -> Result<(), ProfileError> {
        ...
        for (agent, selection) in &self.agents {
            ...
            if *agent != AgentId::Codex && !selection.codex.is_empty() {
                return Err(ProfileError::CodexSettingsOnOtherAgent(*agent));
            }
            for model in &selection.codex.catalog_models {
...
impl TryFrom<UncheckedConnectionProfile> for ConnectionProfile {
    ...
    fn try_from(unchecked: UncheckedConnectionProfile) -> Result<Self, Self::Error> {
        ...
        // Profiles saved before the Buddy adapter was added omit that key.
        agents.entry(AgentId::Workbuddy).or_default();
        if unchecked.schema_version < Self::SCHEMA_VERSION {
            ...
            for (agent, selection) in &mut agents {
                selection.protocol = match agent {
                    ...
                    AgentId::Opencode | AgentId::Workbuddy => Protocol::OpenaiChat,
                };
            }
        }
        let profile = Self {
...
pub enum ProfileError {
    ...
    #[error("unknown Codex {setting} value `{value}`")]
    InvalidCodexSetting {
        setting: &'static str,
        value: String,
    },
    #[error("Codex settings cannot be attached to {0:?}")]
    CodexSettingsOnOtherAgent(AgentId),
...
mod tests {
    ...
    #[test]
    fn the_pre_rename_buddy_spelling_still_reads_back() {
        let profile: BTreeMap<AgentId, u8> =
            serde_json::from_str(r#"{"codebuddy":1,"claude":2}"#).expect("legacy profile key");
        assert_eq!(profile.get(&AgentId::Workbuddy), Some(&1));
        assert_eq!(AgentId::from_id("codebuddy"), Some(AgentId::Workbuddy));
        assert_eq!(AgentId::from_id("workbuddy"), Some(AgentId::Workbuddy));
        assert_eq!(AgentId::from_id("nobody"), None);
        assert_eq!(AgentId::Workbuddy.as_str(), "workbuddy");
    }
    ...
    fn persisted_profile_can_include_credential_secret_redacted_in_debug() {
        ...
        assert_eq!(profile.agents.len(), AgentId::ALL.len());

        let decoded: ConnectionProfile = serde_json::from_str(&json).expect("deserialize profile");
        assert_eq!(decoded, profile);

        let mut old_json: serde_json::Value = serde_json::from_str(&json).expect("profile JSON");
        old_json
            .as_object_mut()
            .expect("profile object")
            .remove("credential_pending");
        let old_profile: ConnectionProfile =
            serde_json::from_value(old_json).expect("profile without pending marker");
        assert!(!old_profile.credential_pending);
    }
    ...
    fn agent_protocol_matrix_and_auto_resolution_are_explicit() {
        ...
        for (agent, expected) in [
            ...
            (AgentId::Workbuddy, WireProtocol::OpenaiChat),
        ] {
            assert_eq!(
                agent
        ...
        // A Gateway that speaks nothing this Agent understands is refused for
        // that Agent alone; the connection itself stays usable for the others.
        assert!(matches!(
            AgentId::Codex.resolve_protocol(
                Protocol::Auto,
                Some(&BTreeSet::from([WireProtocol::Anthropic]))
            ),
            Err(ProfileError::NoCompatibleProtocol(AgentId::Codex))
        ));
    }
