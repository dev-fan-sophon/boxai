        ...
        for redirects in 0..=MAX_REDIRECTS {
            ...
            if response.status().is_redirection() {
                ...
                let next = url
                    .join(location)
                    .map_err(|_| CatalogError::Invalid("invalid redirect".into()))?;
                // An anonymous archive carries no secret to leak and its bytes
                // are pinned by size and digest below, so a delivery network may
                // send it to a regional origin. A bearer archive stays on its
                // own origin: following that redirect would hand the connection
                // credential to whoever answered.
                let cross_origin = next.origin() != origin;
                if (bearer && cross_origin)
                    || (cross_origin && url.scheme() == "https" && next.scheme() != "https")
...
mod tests {
    ...
    #[test]
    fn an_anonymous_archive_follows_a_regional_delivery_redirect_it_can_still_verify() {
        let bytes = skill_zip(&[("SKILL.md", b"regional", 0o644)]);
        let source = Server::http("127.0.0.1:0").expect("delivery server");
        let source_origin = format!("http://{}", source.server_addr());
        let regional = Server::http("127.0.0.1:0").expect("regional server");
        let regional_url = format!("http://{}/regional/archive.zip", regional.server_addr());
        let authenticated = Arc::new(Mutex::new(Vec::new()));
        let observed = Arc::clone(&authenticated);
        let source_handle =
            thread::spawn(move || {
                let request = recv_request(&source, "delivery redirect request");
                ...
                request
                    .respond(Response::empty(StatusCode(307)).with_header(
                        Header::from_bytes("Location", regional_url).expect("Location"),
                    ))
                    .expect("redirect");
            });
        let served = bytes.clone();
        let observed = Arc::clone(&authenticated);
        let regional_handle = thread::spawn(move || {
            let request = recv_request(&regional, "regional archive request");
            observed.lock().expect("observed").push(
                request
                    .headers()
                    .iter()
            ...
            request
                .respond(Response::from_data(served))
                .expect("archive response");
        });
        ...
        let paths = SkillCatalog::new(temp.path().to_owned())
            .expect("catalog")
            .synchronize(
                &manifest(&source_origin, &[&source_origin]),
                &provisioning(vec![skill(
                    "regional",
                    &format!("{source_origin}/archive.zip"),
                    &bytes,
            ...
            .expect("anonymous archives may be delivered from another origin");
        source_handle.join().expect("delivery server");
        regional_handle.join().expect("regional server");

        assert_eq!(
            fs::read(paths["regional"].join("SKILL.md")).expect("regional Skill"),
            b"regional"
        );
        assert_eq!(
            *authenticated.lock().expect("observed"),
            vec![false, false],
            "an anonymous archive must not carry the connection bearer on any hop"
        );
    }
}
