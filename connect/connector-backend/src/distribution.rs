use gateway_connector_core::{AgentId, CanonicalBaseUrl};
use thiserror::Error;
use url::Url;
...
pub struct Distribution {
    ...
    /// Agent adapters this product exposes. The shared core may retain more
    /// adapters for compatible distributions and receipt migration.
    pub supported_agents: &'static [AgentId],
    ...
    pub fn validate(&self) -> Result<(), DistributionError> {
        ...
        if self.supported_agents.is_empty()
            || self
                ...
                .len()
                != self.supported_agents.len()
        {
            return Err(DistributionError::InvalidAgents);
        }
        ...
        if let Some(release) = self.release_metadata {
            if let Some(url) = release.download_url {
                secure_url(url).map_err(|_| DistributionError::InvalidUrl("download_url"))?;
            }
            if let Some(url) = release.update_feed_url {
                secure_url(url).map_err(|_| DistributionError::InvalidUrl("update_feed_url"))?;
            }
        }
        Ok(())
    }
...
pub struct ReleaseMetadata {
    ...
    pub update_feed_url: Option<&'static str>,
}
...
pub enum DistributionError {
    ...
    #[error("distribution supported Agents must be non-empty and unique")]
    InvalidAgents,
