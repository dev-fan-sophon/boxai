...
///
/// The browser half takes seconds when the path works at all. When it does not
/// — a global proxy that does not exempt `127.0.0.1`, security software that
/// refuses the listening socket — nothing will ever arrive, and every extra
/// minute of waiting is a minute the person cannot act on the real problem. So
/// the wait is short enough to reach the explanation while still leaving room
/// to sign in on the platform first.
pub const CALLBACK_TIMEOUT: Duration = Duration::from_secs(180);
const MAX_REQUEST_LINE: usize = 8192;
/// How long one connection may take to say what it wants. It is short because
/// a browser delivering a callback has the whole request ready, and an idle
/// connection is now abandoned rather than fatal.
...
impl PkceFlow {
    ...
    /// Reads one connection to the callback port and decides what it was.
    ///
    /// `Ok(None)` means "that was not the sign-in, keep waiting". Everything
    /// that is not a well-formed callback for this flow lands there, including
    /// every I/O failure on the connection itself. The port is open to
    /// everything on the machine and a browser is the noisiest thing on it: it
    /// opens speculative preconnects and holds them idle, starts private
    /// network preflights and abandons them, races connections and drops the
    /// losers. Reading a failure on one of those as a failed sign-in ends a
    /// login the person has not even completed yet, which is what
    /// `callback I/O failed` used to mean. Only their own callback or the
    /// deadline may end the wait.
    fn read_callback(&self, mut stream: std::net::TcpStream) -> Result<Option<String>, PkceError> {
        // A socket accepted from a nonblocking listener may inherit that mode
        // on some platforms, so reads are put back on a bounded blocking basis.
        ...
        // The reply is a courtesy to whatever is on the other end. It cannot
        // decide the sign-in: by the time we answer, the code is already ours.
        ...
        let Some(callback) = callback.filter(|u| valid_get && u.path() == CALLBACK_PATH) else {
            respond(&mut stream, "404 Not Found", "Ignored callback.");
            return Ok(None);
        };
        let params: std::collections::HashMap<_, _> = callback.query_pairs().into_owned().collect();
        if params.get("state") != Some(&self.state) {
            respond(&mut stream, "400 Bad Request", "Ignored callback.");
            return Ok(None);
        }
        // From here the caller is holding this flow's own state, so what it
        // says is decisive even when it is bad news.
        if let Some(error) = params.get("error") {
            respond(&mut stream, "400 Bad Request", "Login denied.");
        ...
        respond(&mut stream, "200 OK", "Login complete.");
        Ok(Some(code))
    }
}
#[derive(Deserialize)]
...
mod tests {
    ...
    enum Callback {
        ...
        DelayedCode,
        Denied,
        /// A connection that opens and then just sits there. This is what a
        /// speculative preconnect is: the browser opens the socket early and
        /// sends nothing until it decides to navigate, which may be never.
        Silent,
        /// A request the client answers, from a peer that has already gone.
        /// An aborted probe looks like this: the write of our reply fails.
        AbandonedRequest,
    }
            ...
            *self.opened.lock().map_err(|_| PkceError::Browser)? = Some(url.clone());
            ...
            thread::spawn(move || {
                for callback in callbacks {
                    ...
                    if matches!(callback, Callback::Silent) {
                        // Longer than the client's per-connection read timeout,
                        // so the client has to decide what an idle connection
                        // means before the real callback arrives.
                    ...
                    let target = match callback {
                        ...
                        Callback::Silent | Callback::AbandonedRequest => {
                            "/callback?probe=1".to_owned()
                        }
                    };
                    write!(stream, "GET {target} HTTP/1.1\r\n").expect("write callback");
                    if matches!(callback, Callback::AbandonedRequest) {
                        drop(stream);
                        continue;
                    }
    ...
    /// The loopback port is open to anything on the machine, and a browser is
    /// the noisiest thing on it: speculative preconnects, private-network
    /// preflights it abandons, probes it cancels. None of that is the person
    /// signing in, and none of it may end their sign-in.
    #[test]
    fn stray_connections_to_the_callback_port_do_not_end_the_sign_in() {
        let (token_url, token_handle) = spawn_token_response(
            200,
            r#"{"access_token":"durable-access","token_type":"Bearer"}"#,
            None,
        );
        let browser = CallbackBrowser::new(vec![
            Callback::Silent,
            Callback::AbandonedRequest,
            Callback::Code,
        ]);
        ...
        let token = flow
            ...
            .expect("a sign-in must survive connections that are not the callback");

        assert_eq!(token.expose_secret(), "durable-access");
        token_handle.join().expect("token request");
    }
    ...
    #[test]
    fn a_callback_that_never_arrives_ends_as_a_timeout_and_not_as_a_hang() {
        ...
        // The browser "opened" and then nothing reached the loopback port, which
        // is exactly what a proxy or a firewall produces.
        let browser = CallbackBrowser::new(vec![]);
        let error = PkceFlow::from_parts("verifier".into(), "state".into())
            .login_within(
                &Url::parse("https://platform.example/authorize").expect("authorize URL"),
        ...
        assert!(matches!(error, PkceError::Timeout), "{error}");
        assert!(
            token_server
                .recv_timeout(Duration::from_millis(250))
                .is_ok_and(|request| request.is_none())
