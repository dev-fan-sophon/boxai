...
use crate::{
    ...
    GENERIC_DISTRIBUTION, GatewayClient, ModelCapability, ModelDescriptor, Overview, ProfileStore,
...
impl ConnectorBackend {
    ...
    /// Configures provisioned Skill state and the neutral Agent runtime.
    ...
    pub fn discover_agents(&self) -> Result<Vec<AgentInstall>, BackendError> {
        let runtime = self
            ...
            .as_ref()
            .ok_or(BackendError::ProjectionNotConfigured)?;
        Ok(self.discovered_agents(runtime))
    }

    fn discovered_agents(&self, runtime: &ProjectionRuntime) -> Vec<AgentInstall> {
        runtime
            .discovery
            .discover()
            .into_iter()
            .filter(|install| self.distribution.supported_agents.contains(&install.agent))
            .collect()
    }
    ...
    pub fn plan_projection(&self, connection: &ConnectionResult) -> Result<Plan, BackendError> {
        self.plan_projection_for(connection, None)
    }

    pub fn plan_agent_projection(
        &self,
        connection: &ConnectionResult,
        agent: AgentId,
    ) -> Result<Plan, BackendError> {
        self.plan_projection_for(connection, Some(agent))
    }

    fn plan_projection_for(
        &self,
        connection: &ConnectionResult,
        only: Option<AgentId>,
    ) -> Result<Plan, BackendError> {
        ...
        let bearer = self
            ...
            .ok_or(BackendError::MissingCredential)?;
        let secret = core_secret(&bearer)?;
        let installs = self.discovered_agents(runtime);
        ...
        let agents = installs
            .iter()
            .filter(|install| install.detected && only.is_none_or(|agent| install.agent == agent))
            .map(|install| {
                ...
                Ok((
                    ...
                    EffectiveAgentSelection {
                        model,
                        protocol,
                        disabled_mcp: selection.disabled_mcp.clone(),
                        disabled_skills: selection.disabled_skills.clone(),
                        codex: selection.codex.clone(),
                        image_direct: selection.image_direct,
                    },
                ))
            })
            .collect::<Result<BTreeMap<_, _>, ProfileError>>()?;
        if let Some(agent) = only
            && !agents.contains_key(&agent)
        {
            return Err(BackendError::ProjectionContext(format!(
                "{} is not a detected supported Agent",
                agent.display_name()
            )));
        }
        runtime
        ...
        let (
            ...
            ProbeResult::Provisioned {
                ...
                let (provisioning, synchronized_skills) =
                    self.provisioned_catalog(&manifest, &request.api_key)?;
    ...
    pub fn browser_login(
        ...
        let mut profile = ConnectionProfile::new_connection(
        ...
        let pending = PendingCredential {
            token,
            profile: {
                profile.credential_pending = true;
                profile.clone()
            },
        ...
        *pending_guard = Some(pending.clone());
        ...
        let profile = self.finish_credential_commit(&profile)?;
        *pending_guard = None;
        drop(pending_guard);
    ...
    /// The single sign-in entry point of a distribution that authenticates only
    /// through the browser. A machine that already holds a saved connection
    /// resumes it rather than starting a second login the store would refuse:
    /// the signed-out screen offers no way to disconnect, so reporting
    /// [`BackendError::AlreadyConnected`] there strands the person for good.
    pub fn sign_in(
        &self,
        base_url: &str,
        display_name: String,
    ) -> Result<ConnectionResult, BackendError> {
        if let Some(resumed) = self.resume_saved()? {
            return Ok(resumed);
        }
        let ProbeResult::Provisioned {
            manifest_url,
            manifest,
            ..
        } = self.probe(base_url)?
        else {
            return Err(BackendError::ManifestHasNoBrowserAuth);
        };
        if manifest.authentication.is_none() {
            return Err(BackendError::ManifestHasNoBrowserAuth);
        }
        self.browser_login(BrowserLoginOffer {
            request: ConnectRequestWithoutCredential {
                display_name,
    ...
    pub fn fetch_overview(
        ...
        profile: &ConnectionProfile,
        manifest: &ConnectionManifest,
    ) -> Result<Overview, BackendError> {
        let bearer = self
            .credentials
            .get(profile)?
            .ok_or(BackendError::MissingCredential)?;
        Ok(self.client.fetch_overview(manifest, &bearer))
    }
    ...
    pub fn retry_pending_credential(&self) -> Result<ConnectionResult, BackendError> {
        ...
        let profile = self.finish_credential_commit(&pending.profile)?;
        *self
            .pending_credential
            .lock()
    ...
    fn finish_credential_commit(
        &self,
        profile: &ConnectionProfile,
    ) -> Result<ConnectionProfile, BackendError> {
        let mut persisted = self
            .single_profile()?
            .filter(|saved| saved.id == profile.id)
            .ok_or(StoreError::ActiveProfileExists)?;
        if persisted.credential_pending {
            persisted.credential_pending = false;
            self.profiles.save(&persisted)?;
        }
        Ok(persisted)
    }
...
fn models_from_provisioning(value: &Provisioning) -> Vec<ModelDescriptor> {
    let mut models: Vec<_> = value
        ...
        .map(|m| ModelDescriptor {
            ...
            metadata: if m.supported_reasoning.is_empty() {
                BTreeMap::new()
            } else {
                BTreeMap::from([(
                    "supported_reasoning".into(),
                    serde_json::json!(m.supported_reasoning),
                )])
            },
        })
        .collect();
