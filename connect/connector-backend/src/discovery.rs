...
use gateway_connector_core::{
    CanonicalBaseUrl, ConnectionManifest, Provisioning, RequestLogEntry, UsageBucket, UsageStat,
};
...
/// The time-window readings provisioning cannot carry. Account, wallet, and
/// subscription state come from the provisioning document instead.
#[derive(Debug, Clone, PartialEq)]
pub struct Overview {
    pub usage_stat: Result<UsageStat, String>,
    pub usage_series: Result<Vec<UsageBucket>, String>,
    pub recent_logs: Result<Vec<RequestLogEntry>, String>,
}

const USAGE_SERIES_SECONDS: i64 = 30 * 24 * 60 * 60;
...
impl GatewayClient {
    ...
    pub fn fetch_overview(&self, manifest: &ConnectionManifest, bearer: &ApiKey) -> Overview {
        let end = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .map(|duration| duration.as_secs() as i64)
            .unwrap_or(0);
        let start = end.saturating_sub(USAGE_SERIES_SECONDS);
        Overview {
            usage_stat: self
                .fetch_usage_stat(manifest, bearer)
                .map_err(|error| error.to_string()),
            usage_series: self
                .fetch_usage_series(manifest, bearer, start, end)
                .map_err(|error| error.to_string()),
            recent_logs: self
                .fetch_recent_logs(manifest, bearer)
                .map_err(|error| error.to_string()),
        }
    }

    pub fn fetch_usage_stat(
        &self,
        manifest: &ConnectionManifest,
        bearer: &ApiKey,
    ) -> Result<UsageStat, DiscoveryError> {
        self.fetch_self_json(manifest, bearer, "api/log/self/stat", &[], UsageStat::parse)
    }

    pub fn fetch_usage_series(
        &self,
        manifest: &ConnectionManifest,
        bearer: &ApiKey,
        start: i64,
        end: i64,
    ) -> Result<Vec<UsageBucket>, DiscoveryError> {
        if end < start || end - start > USAGE_SERIES_SECONDS {
            return Err(DiscoveryError::InvalidOverview(
                "usage series must cover at most 30 days".into(),
            ));
        }
        self.fetch_self_json(
            manifest,
            bearer,
            "api/data/self",
            &[
    ...
    pub fn fetch_recent_logs(
        ...
        manifest: &ConnectionManifest,
        bearer: &ApiKey,
    ) -> Result<Vec<RequestLogEntry>, DiscoveryError> {
        self.fetch_self_json(
            manifest,
            bearer,
            "api/log/self",
            &[("p", "1".into()), ("page_size", "100".into())],
            RequestLogEntry::parse_page,
        )
    }

    fn fetch_self_json<T>(
        &self,
        manifest: &ConnectionManifest,
        bearer: &ApiKey,
        path: &str,
        query: &[(&str, String)],
        parse: impl FnOnce(&[u8]) -> Result<T, gateway_connector_core::Error>,
    ) -> Result<T, DiscoveryError> {
        let mut url =
            manifest
                .api_endpoint(path)
                .map_err(|error| DiscoveryError::InvalidManifest {
                    message: error.to_string(),
                })?;
        if !query.is_empty() {
        ...
        let response = self.get_following_exact_origin(url, &base, Some(bearer.expose_secret()))?;
        if !response.status().is_success() {
            return Err(DiscoveryError::OverviewHttpStatus(
                response.status().as_u16(),
            ));
        }
        let bytes = read_bounded(response)?;
        parse(&bytes).map_err(|source| DiscoveryError::InvalidOverview(source.to_string()))
    }
...
pub enum DiscoveryError {
    ...
    #[error("the overview endpoint returned HTTP {0}")]
    OverviewHttpStatus(u16),
    #[error("overview is invalid: {0}")]
...
#[cfg(test)]
mod tests {
    use gateway_connector_core::{Gateway, Platform, WireProtocol};

    use super::*;
    use crate::ApiKey;

    #[test]
    fn usage_series_rejects_spans_over_thirty_days() {
        let client = GatewayClient::new().expect("client");
        let origin = Url::parse("https://gateway.example.test").expect("origin");
        let manifest = ConnectionManifest::direct(
            ...
            Gateway {
                ...
                protocols: vec![WireProtocol::Anthropic],
            },
            origin
                .join("api/connector/provisioning")
                .expect("provisioning"),
        )
        .expect("manifest");
        let key = ApiKey::new("sk-test").expect("key");
        let error = client
            .fetch_usage_series(&manifest, &key, 0, USAGE_SERIES_SECONDS + 1)
            .expect_err("span");
        assert!(matches!(error, DiscoveryError::InvalidOverview(_)));
    }
}
