...
use gateway_connector_backend::{
    ...
    CredentialStore, DiscoveryError, Distribution, GENERIC_DISTRIBUTION, GatewayClient,
    InMemoryCredentialStore, InMemoryProfileStore, ModelCapability, ModelDescriptor, PkceError,
...
fn manifest_body(
    ...
    serde_json::json!({
        ...
        "data": {
            ...
            "supported_agents": ["claude", "codex", "gemini", "grokbuild", "opencode", "workbuddy"]
        }
    })
    .to_string()
}
    ...
    .and_then(|backend| {
        backend.with_agent_root_overrides(
            [
                ...
                (AgentId::Workbuddy, home.path().join(".workbuddy")),
            ]
            .into_iter()
            .collect(),
        )
...
#[test]
fn distribution_exposes_only_its_supported_agents() {
    let state = tempdir().expect("projection state");
    let home = tempdir().expect("Agent home");
    fs::create_dir(home.path().join(".codex")).expect("Codex root");
    fs::create_dir(home.path().join(".workbuddy")).expect("WorkBuddy root");
    let distribution = Box::leak(Box::new(Distribution {
        supported_agents: &[AgentId::Codex],
    ...
    assert_eq!(installs[0].agent, AgentId::Codex);
...
fn browser_login_keeps_failed_vault_credentials_retryable() {
    ...
    let recoverable = backend.profiles().expect("recoverable profile");
    assert_eq!(recoverable.len(), 1);
    assert!(recoverable[0].credential_pending);
    ...
    assert!(!backend.has_pending_credential().expect("pending state"));
    assert!(!connected.profile.credential_pending);
    assert_eq!(
...
#[test]
fn resume_saved_discards_interrupted_browser_profile_without_credential() {
    let credentials = Arc::new(InMemoryCredentialStore::default());
    let profiles = Arc::new(InMemoryProfileStore::default());
    let mut interrupted = ConnectionProfile::new_connection(
        "Interrupted login",
        CanonicalBaseUrl::parse("https://gateway.example").expect("base URL"),
        ConnectionMode::Provisioned,
        CredentialKind::AccessToken,
        "gateway-connector",
        Some(Url::parse("https://gateway.example/connector-manifest.json").expect("manifest URL")),
    )
    .expect("profile");
    interrupted.credential_pending = true;
    profiles.save(&interrupted).expect("save pending profile");

    let backend = ConnectorBackend::new(credentials, profiles).expect("backend");
    assert!(backend.resume_saved().expect("recover startup").is_none());
    assert!(backend.profiles().expect("profiles").is_empty());
}

#[test]
fn resume_saved_finishes_interrupted_credential_commit() {
    let (base, handle) = spawn_direct(r#"{"data":[{"id":"model-a"}]}"#, 1);
    let credentials = Arc::new(InMemoryCredentialStore::default());
    let profiles = Arc::new(InMemoryProfileStore::default());
    let mut interrupted = ConnectionProfile::new(
        "Interrupted commit",
        CanonicalBaseUrl::parse(&base).expect("base URL"),
    )
    .expect("profile");
    ...
    profiles.save(&interrupted).expect("save pending profile");
    credentials
        .set(&interrupted, &ApiKey::new("committed-key").expect("key"))
        .expect("commit credential");
    ...
    handle.join().expect("mock server");
    assert_eq!(resumed.profile.id, interrupted.id);
    assert!(!resumed.profile.credential_pending);
    assert!(!backend.profiles().expect("profiles")[0].credential_pending);
}

#[test]
fn resume_saved_preserves_unmarked_profile_without_credential() {
    let credentials = Arc::new(InMemoryCredentialStore::default());
    let profiles = Arc::new(InMemoryProfileStore::default());
    ...
    let backend = ConnectorBackend::new(credentials, profiles).expect("backend");
    assert!(matches!(
        backend.resume_saved(),
        Err(BackendError::MissingCredential)
    ));
    assert_eq!(backend.profiles().expect("profiles"), vec![profile]);
}
...
#[test]
fn signing_in_again_resumes_the_saved_connection_instead_of_refusing_it() {
    let server = Server::http("127.0.0.1:0").expect("sign-in server");
    let origin = format!("http://{}", server.server_addr());
    let manifest_url = leak_str(format!("{origin}/connector-manifest.json"));
    let distribution = pinned_distribution(manifest_url);
    let manifest = browser_manifest_body("pinned-platform", &origin);
    ...
    let handle = thread::spawn(move || {
        ...
        for _ in 0..5 {
            let request = recv_request(&server, "sign-in request");
            let path = request.url().to_owned();
            let body = match path.as_str() {
                "/token" => r#"{"access_token":"browser-access","token_type":"Bearer"}"#.to_owned(),
                "/api/connector/provisioning" => provisioning.clone(),
                _ => manifest.clone(),
            };
            paths.push(path);
    ...
    let first = backend
        .sign_in(&origin, "Origin Gateway".to_owned())
        .expect("browser sign-in");
    // Pressing the same button again is what strands a person whose first
    // attempt saved a profile but failed afterwards.
    let second = backend
        .sign_in(&origin, "Origin Gateway".to_owned())
        .expect("a saved connection resumes");

    let paths = handle.join().expect("sign-in server");
    assert_eq!(
        paths,
        [
            "/connector-manifest.json",
    ...
    assert_eq!(second.models[0].id, "agent-model");
    assert_eq!(
        browser.opened.load(Ordering::SeqCst),
        1,
        "resuming must not open a second browser login"
    );
    assert_eq!(backend.profiles().expect("profiles").len(), 1);
}
