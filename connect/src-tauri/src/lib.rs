...
mod app_store;
mod auto_launch;
// Public so the integration tests can project the seeded provider payloads through
// the real client config writers.
pub mod boxai;
...
// Public alongside hermes_config: the integration tests project the seeded provider
// payload through this writer to prove the shape the live config path expects.
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // 初始化日志（输出到 <app_config_dir>/logs/cc-switch.log）
            {
                    ...
                    tauri_plugin_log::Builder::default()
                        ...
                        .targets([
                            ...
                            Target::new(TargetKind::Folder {
                                path: log_dir,
                                file_name: Some("boxai-connect".into()),
                            }),
                        ])
                ...
                log::info!("=== BoxAI Connect v{} started ===", env!("CARGO_PKG_VERSION"));
            }
            ...
            // Clone before manage so startup tasks never call AppHandle::state()
            // during setup (that path has panicked with "state() called before
            // manage()" on a tokio worker even after manage was reordered first).
            let startup_state = app_state.clone();
            let proxy_restore_state = app_state.clone();
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            // BoxAI account: sign-in state and per-client model profiles
            boxai::gateway_auth::gateway_auth_status,
...
fn show_migration_error_dialog(app: &tauri::AppHandle, error: &str) -> bool {
    ...
    let message = if is_chinese_locale() {
        ...
        format!(
            ...
            Consider rolling back to an older BoxAI Connect version.\n\n\
