...
/// 获取应用配置目录路径 (~/.boxai-connect)
///
/// Deliberately not `~/.cc-switch`: Connect is a separate product with its own
/// bundle identifier, and a user who also runs upstream CC Switch must not end
/// up with two applications writing one SQLite database.
pub fn get_app_config_dir() -> PathBuf {
    if let Some(custom) = crate::app_store::get_app_config_dir_override() {
        return custom;
    }

    get_home_dir().join(".boxai-connect")
}
