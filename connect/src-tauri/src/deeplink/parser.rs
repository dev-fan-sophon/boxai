...
//!
//! Parses boxaiconnect:// and ccswitch:// URLs into DeepLinkImportRequest
//! structures.
...
/// The scheme Connect registers with the OS.
const CONNECT_SCHEME: &str = "boxaiconnect";
/// Upstream's scheme, still accepted so links published for CC Switch import
/// unchanged. Connect does not register it, so it never takes the scheme away
/// from a real CC Switch install; it only handles what the OS routes here.
const UPSTREAM_SCHEME: &str = "ccswitch";

/// Parse a deep link URL into a DeepLinkImportRequest
///
/// Expected format:
/// boxaiconnect://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if scheme != CONNECT_SCHEME && scheme != UPSTREAM_SCHEME {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected '{CONNECT_SCHEME}', got '{scheme}'"
        )));
    }
