use connector_core::{
    AgentId, AgentInstall, ApplyInput, ConnectionManifest, Connector, Provisioning, Secret,
};
...
fn setup(root: &Path) -> (Connector, connector_core::Plan) {
    let (m, p) = contracts();
...
fn projects_all_clients_preserves_and_disconnects_owned_entries() {
    ...
    let mcp = managed("origin", "mcp", "docs");
    let skill = "deploy";
    ...
    assert!(!t.path().join("state/receipts/origin.json").exists());
    assert_eq!(
...
fn independently_branded_connectors_share_agent_ownership_leases() {
    ...
    let origin = Connector::with_coordinator(t.path().join("origin-state"), &coordinator);
    let origin_plan = origin
        .plan(ApplyInput {
        ...
        .unwrap();
    origin.apply(&origin_plan).unwrap();

    let mut box_manifest = origin_manifest.clone();
    box_manifest.platform.id = "boxai".into();
    box_manifest.platform.name = "BoxAI".into();
    let box_connector = Connector::with_coordinator(t.path().join("box-state"), &coordinator);
    let error = box_connector
        .plan(ApplyInput {
    ...
    origin
        .disconnect("origin", &Secret::new("origin-secret").unwrap())
        .unwrap();
    let box_plan = box_connector
        .plan(ApplyInput {
            manifest: &box_manifest,
            provisioning: &provisioning,
            bearer: &Secret::new("box-secret").unwrap(),
...
fn skill_replacement_requires_both_owner_marker_and_framed_tree_hash() {
    ...
    let error = connector
        .disconnect("origin", &Secret::new("secret").unwrap())
        .unwrap_err()
        .to_string();
...
fn reapply_keeps_first_original_and_drift_disconnect_is_semantic() {
    ...
    c.disconnect("origin", &Secret::new("super-secret").unwrap())
        .unwrap();
    assert_eq!(
    ...
    assert!(after["provider"][managed("origin", "provider", "default")].is_null());
}
...
fn reapply_removes_stale_mcp_and_skill_ownership() {
    ...
    assert!(
        second
            .changes
            .iter()
            .any(|change| change.kind == connector_core::ChangeKind::Remove
...
fn drifted_text_projection_blocks_disconnect_and_keeps_ownership() {
    ...
    let error = connector
        .disconnect("origin", &Secret::new("super-secret").unwrap())
        .unwrap_err()
        .to_string();
    assert!(error.contains("local changes"), "{error}");
...
fn malformed_receipt_nonce_is_rejected_without_panicking() {
    ...
    let (connector, plan) = setup(t.path());
    connector.apply(&plan).unwrap();
    let path = t.path().join("state/receipts/origin.json");
...
fn receipt_is_authenticated_ciphertext_and_verification_rejects_two_missing_directories() {
    ...
    let (connector, plan) = setup(t.path());
    connector.apply(&plan).unwrap();
    let receipt = fs::read(t.path().join("state/receipts/origin.json")).unwrap();
    assert!(
        !receipt
    ...
    fs::remove_dir_all(t.path().join("state/skills/origin/deploy")).unwrap();
