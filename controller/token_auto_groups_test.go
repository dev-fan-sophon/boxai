...
func TestAddTokenPersistsOrderedAutoGroupsSnapshot(t *testing.T) {
	...
	getCtx, getRecorder := newTokenAutoGroupsAuthenticatedContext(t, http.MethodGet, "/api/token/"+strconv.Itoa(token.Id), nil, user.Id)
	getCtx.Params = append(getCtx.Params, gin.Param{Key: "id", Value: strconv.Itoa(token.Id)})
	GetToken(getCtx)
...
func TestGetTokenAutoGroupsReturnsFilteredGlobalOrderAndLimit(t *testing.T) {
	...
	assert.NotContains(t, fmt.Sprint(data.Groups), "missing")
}
