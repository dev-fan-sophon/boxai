...
var auditContentTemplates = map[string]string{
	...
	"reward.campaign_create": "Created reward campaign ${name} (${slug}, ${quota})",
	"reward.campaign_update": "Updated reward campaign ${name} (ID: ${id}, ${slug}, ${quota})",
...
func recordManageAuditFor(c *gin.Context, targetUserId int, action string, params map[string]interface{}) {
	...
	model.RecordOperationAuditLog(operatorUserId, auditContentEN(action, params), common.RealClientIP(c), action, params, auditOperatorInfo(c), nil)
	markAuditLogged(c)
}
...
func recordUserSecurityAudit(c *gin.Context, userId int, action string, params map[string]interface{}) {
	model.RecordOperationAuditLog(userId, auditContentEN(action, params), common.RealClientIP(c), action, params, nil, nil)
}
