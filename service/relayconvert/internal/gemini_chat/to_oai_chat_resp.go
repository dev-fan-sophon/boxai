...
func (s *GeminiToChatStreamState) ConvertChunk(geminiResponse *dto.GeminiChatResponse, model string, usage *dto.Usage) []*dto.ChatCompletionsStreamResponse {
	...
	if response.IsToolCall() {
		...
		if !hasNonStopFinish {
			for i := range response.Choices {
				if response.Choices[i].FinishReason != nil && *response.Choices[i].FinishReason == constant.FinishReasonToolCalls {
	...
	if isStop && !s.finishEmitted {
		responses = append(responses, s.terminalChunk())
	}
	return responses
}

func (s *GeminiToChatStreamState) Finalize() []*dto.ChatCompletionsStreamResponse {
	if s == nil || s.finishEmitted {
		return nil
	}
	return []*dto.ChatCompletionsStreamResponse{s.terminalChunk()}
}
...
func (s *GeminiToChatStreamState) terminalChunk() *dto.ChatCompletionsStreamResponse {
	finishReason := constant.FinishReasonStop
	if s.sawToolCall {
